...
from revolve2.simulators.mjx_simulator._scene_to_model import scene_to_model
...
#     handlers=[
#         logging.FileHandler('gpu_examples/1a_simulate_single.log'), # Log messages to a file
...
def create_batch(scenes, batch_parameters, record_settings):        # simulate_scenets from modular robot simulation
    ...
    return batch, modular_robot_to_multi_body_system_mappings
...
def main():
    scenes = make_scene()
    batch, modular_robot_to_multi_body_system_mappings = create_batch(scenes, make_standard_batch_parameters(), None)
    # print(batch)
    model, mapping = scene_to_model(batch.scenes[0], batch.parameters.simulation_timestep, cast_shadows=True, fast_sim=True)
    ...
    media.write_video("1a_2.mp4", frames, fps=framerate)
